...
exports.createStdProfilePromise = async (req, res) => {
    try {
        ...
        res.redirect(`/promise/${stdNum}/${promise.promiseNum}`);
    } catch (error) {
...
exports.showPromiseConfirmation = async (req, res) => {
    try {
        ...
        // 약속 정보를 템플릿에 전달합니다.
        res.render('stdPromiseRequest', { stdNum: stdNum, promise: promise });
    } catch (error) {
...
exports.acceptPromise = async (req, res) => {
    try {
        ...
        res.status(200).json({ redirectUrl: `/promise/${stdNum}/${promiseNum}/deposit` });
    } catch (error) {
...
exports.showPromiseDeposit = async (req, res) => {
    try {
        ...
        promise.formattedPromiseDay = formattedDate;
//목록페이지로 리다이렉트
        res.render('stdPromiseDeposit', { stdNum: stdNum, promise: promise });
    } catch (error) {
        console.error('Error showing promise page:', error);
        res.status(500).json({ error: '페이지를 불러오는 동안 오류가 발생했습니다.' });
    }
